...
local o = {
    ...
    gkey_scroll_up = "GAMEPAD_DPAD_UP",
    gkey_scroll_down = "GAMEPAD_DPAD_DOWN",
    --key_scroll_up = "GAMEPAD_DPAD_UP",
    --key_scroll_down = "GAMEPAD_DPAD_DOWN",
    scroll_lines = 1,
    ...
    -- Text style
    font = "unifont",
    font_mono = "unifont",   -- monospaced digits are sufficient
    font_size = 8,
    font_color = "FFFFFF",
    border_size = 0.5,
    border_color = "262626",
...
local function text_style(page)
    if page then
        o.alignment = page
    else 
        o.alignment = 4
    ...
    if o.custom_header and o.custom_header ~= "" then
        ...
        return format("{\\r}{\\an%d}{\\fs%d}{\\fn%s}{\\bord%f}{\\3c&H%s&}" ..
                        "{\\1c&H%s&}{\\alpha&H%s&}{\\xshad%f}{\\yshad%f}{\\4c&H%s&}",
                        o.alignment, o.font_size, o.font, o.border_size,
...
local function append_proptime(s, prop, attr, excluded)
    excluded = excluded or {[""] = true}
    if prop == "time-pos" then
        if mp.get_property_number("time-pos") then
            ret = math.floor(mp.get_property_number("time-pos")* 100) / 100 
        end
    else
         ret = mp.get_property_number(prop)
    end
        if not ret or excluded[ret] then
        if o.debug then
            print("No value for property: " .. prop)
...
local function get_kbinfo_lines(width)
    ...
    for _, bind in pairs(bindings) do
        ...
        if bind.priority >= 0 and (
           ...
           ) and not bind.cmd:find("script-binding stats/__forced_", 1, true) and bind.section ~= "input"
        then
            if string.find(bind.key, "GAMEPAD") then
                bind.key = bind.key:gsub("(%w+)", {["Shift"] = "R1", ["Meta"] = "R2", ["Ctrl"] = "L1", ["Alt"] = "L2"})
                bind.key = bind.key:gsub("GAMEPAD_RIGHT_STICK", "R")
                                   :gsub("GAMEPAD_LEFT_STICK", "L")
                                   :gsub("GAMEPAD_DPAD_", "")
                                   :gsub("GAMEPAD_ACTION_DOWN", "\226\155\140") --"ⵝ")⨯
                                   :gsub("GAMEPAD_ACTION_UP", "  \226\131\164")
                                   :gsub("GAMEPAD_ACTION_RIGHT", "  \226\131\157")
                                   :gsub("GAMEPAD_ACTION_LEFT", "  \226\131\158")
                                   :gsub("GAMEPAD_START", "START")
                                   :gsub("GAMEPAD_BACK", "SELECT")
                                   :gsub("GAMEPAD_MENU", "PS")
                --print(type(tostring(bind.key)), type(bind.key))▲ ■ ● ✖
    ...
    local spre = term and kspaces .. "   "
                       or format("{\\q2\\fn%s}%s   {\\fn%s}{\\fs%d\\u1}",
                                 o.font_mono, " ", o.font, 1.3*o.font_size)
    ...
    for _, bind in ipairs(ordered) do
        ...
        if bind.comment then
            ...
            local x = string.char(string.byte("\226\155\140",1,-1))
            local o = string.char(string.byte("\226\131\157",1,-1))
            local t = string.char(string.byte("\226\131\164",1,-1))
            local s = string.char(string.byte("\226\131\158",1,-1))
            if string.match(bind.key, x) then 
                bind.cmd = " # " .. bind.comment
            elseif string.match(bind.key, o) or string.match(bind.key, t) or string.match(bind.key, s) then 
                bind.cmd = "   # " .. bind.comment
            else
                bind.cmd = "  # " .. bind.comment
            end
...
local function add_header(s, page)
    s[#s+1] = text_style(page)
end
...
local function keybinding_info(after_scroll)
    ...
    eval_ass_formatting()
    add_header(header, "7")
    append(header, "", {prefix=o.nl .. o.nl .. page.desc .. ":                                        ", nl="", indent=""})
...
local function frame_stats(stats)
    local stats = {}
    eval_ass_formatting()
    add_header(stats, "0")
    local page = pages[o.key_page_1]
    
    --local fmtsec = parseTime(seconds)
    --append(stats, "", {prefix=o.nl .. o.nl .. page.desc .. ":", nl="", indent=""})
    append_property(stats, "video-frame-info/picture-type", {prefix="type:  ", prefix_sep="", nl="", indent=""})
    append_property(stats, "estimated-frame-number", {prefix="frame: ", prefix_sep="", nl=o.nl, indent=""})
    append_proptime(stats, "time-pos", {prefix="sec:   ", prefix_sep="", nl=o.nl, indent=""})
    --append(stats, "", {prefix=o.nl, nl="", indent=""})
    --page.offset = append_general_perfdata(stats, page.offset)
    return table.concat(stats)
end
...
pages = {
    [o.key_page_5] = { f = default_stats, desc = "Default" },
    [o.key_page_2] = { f = vo_stats, desc = "Extended Frame Timings", scroll = true },
    [o.key_page_3] = { f = cache_stats, desc = "Cache Statistics" },
    [o.key_page_4] = { f = keybinding_info, desc = "Input Map", scroll = true },
    [o.key_page_1] = { f = frame_stats, desc = "Frame info" },
...
local function bind_scroll()
    if not scroll_bound then
        ...
        mp.add_forced_key_binding(o.gkey_scroll_up, "__forced_"..o.gkey_scroll_up, scroll_up, {repeatable=true})
        mp.add_forced_key_binding(o.gkey_scroll_down, "__forced_"..o.gkey_scroll_down, scroll_down, {repeatable=true})
        scroll_bound = true
    end
end
local function unbind_scroll()
    if scroll_bound then
        ...
        mp.remove_key_binding("__forced_"..o.gkey_scroll_up)
        mp.remove_key_binding("__forced_"..o.gkey_scroll_down)
        scroll_bound = false
    end
end
...
tot = tablelength(pages)


--pages nav
function cycle()
    --print('______stats' .. active)
    if active == nil then
        --print('stats', active)
        ...
        if (next == tot) then
            ...
            active = nil
            --curr_page = o.key_page_1
            --update_scroll_bindings(o.key_page_1)
        elseif (next < 1) then
            curr_page = o['key_page_'..tot-1]
            --update_scroll_bindings(next)
        else
            curr_page= o['key_page_'..next]
            --update_scroll_bindings(next)
        end  
        update_scroll_bindings(curr_page)
    end
...
mp.set_property("script-opts", "osc-visibility=always,osc-layout=topbar,osc-seekbarstyle=diamond,osc-hidetimeout=1500,osc-fadeduration=200,osc-timems=yes,osc-scalewindowed=1,osc-scalefullscreen=0.65")
--mp.command("script-opts=osc-visibility=always")
mp.add_forced_key_binding(nil, "page-", function() nav(-1) end, {repeatable=false})
mp.add_forced_key_binding(nil, "page+", function() nav(1) end, {repeatable=false})
mp.add_forced_key_binding(nil, "page++", cycle, {repeatable=false})
mp.add_forced_key_binding(nil, "close", close, {repeatable=false})
