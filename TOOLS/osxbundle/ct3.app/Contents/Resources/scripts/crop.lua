local opts = {
    ...
    mouse_support = true,
    coarse_movement = 30,
    left_coarse = "LEFT,GAMEPAD_DPAD_LEFT",
    right_coarse = "RIGHT,GAMEPAD_DPAD_RIGHT",
    up_coarse = "UP,GAMEPAD_DPAD_UP",
    down_coarse = "DOWN,GAMEPAD_DPAD_DOWN",
    fine_movement = 1,
    left_fine = "Meta+LEFT,Ctrl+GAMEPAD_DPAD_LEFT",
    right_fine = "Meta+RIGHT,Ctrl+GAMEPAD_DPAD_RIGHT",
    up_fine = "Meta+UP,Ctrl+GAMEPAD_DPAD_UP",
    down_fine = "Meta+DOWN,Ctrl+GAMEPAD_DPAD_DOWN",
    accept = "ENTER,MOUSE_BTN0,GAMEPAD_ACTION_RIGHT",
    cancel = "ESC,GAMEPAD_ACTION_UP,GAMEPAD_BACK",
    rev = "Shift+GAMEPAD_ACTION_RIGHT,Shift+p",
    reset = "GAMEPAD_ACTION_UP,z",
    undo = "Meta+GAMEPAD_BACK,Ctrl+z"
}
...
function update_crop_zone_state()
    ...
    if crop_first_corner == nil then
        crop_first_corner_xy = firstp(crop_cursor)
        crop_first_corner = screen_to_video_norm(crop_cursor, dim)
        redraw()
    elseif  crop_first_corner_xy.x == crop_cursor.x and crop_first_corner_xy.y == crop_cursor.y then
        crop_first_corner = nil
        crop_first_corner_xy = nil
        redraw()
    else
...
function rev_crop()
    ...
    if crop_first_corner ~= nil then
        ...
        -- reset first 
                crop_first_corner = nil
                crop_cursor = crop_first_corner_xy
                redraw() 
        
        -- subtitute first with cursor 
                crop_first_corner = nil
                update_crop_zone_state()
        --]]
        local lastp = crop_first_corner_xy
        crop_first_corner_xy = firstp(crop_cursor)
        crop_first_corner = screen_to_video_norm(crop_cursor, dim)
        crop_cursor = lastp
        redraw()
    else
        mp.osd_message("Missing A")
    end
end
function reset_crop()
    local dim = mp.get_property_native("osd-dimensions")
...
function undo_crop()
        ...
        if #vf_table > 0 then
            for i = #vf_table, 1, -1 do
                if vf_table[i].name == to_remove then
                    ...
                    mp.set_property_native("vf", vf_table)
                    --mp.set_property_native("vf", '[{"params":{"x":"346","y":"148","h":"353","w":"494"},"name":"crop"}]')
                    tprint(mp.get_property_native("vf"))
                    return true
                end
            end
        end
    ...
    if not remove_filter() then
        mp.osd_message('No crop applied', 3)
    end
    --mp.set_property("vf", "crop=x=0;y=0;h=0;w=0")
    --mp.commandv("loadfile", mp.get_property('path'), "replace", "start=" .. mp.get_property('time-pos'))
end
...
for _, key in ipairs(input.accept) do
    bindings[key] = update_crop_zone_state
end
for _, key in ipairs(input.cancel) do
    bindings[key] = cancel_crop
end
for _, key in ipairs(input.rev) do
    bindings[key] = rev_crop
end
for _, key in ipairs(input.reset) do
    bindings[key] = reset_crop
end
for _, key in ipairs(input.undo) do
    bindings[key] = undo_crop
end
...
for _, key in ipairs(input.left_coarse) do
    bindings_repeat[key]  = movement_func(-opts.coarse_movement, 0)
end
for _, key in ipairs(input.right_coarse) do
    bindings_repeat[key] = movement_func(opts.coarse_movement, 0)
end
for _, key in ipairs(input.up_coarse) do
    bindings_repeat[key]    = movement_func(0, -opts.coarse_movement)
end
for _, key in ipairs(input.down_coarse) do
    bindings_repeat[key]  = movement_func(0, opts.coarse_movement)
end
for _, key in ipairs(input.left_fine) do
    bindings_repeat[key]    = movement_func(-opts.fine_movement, 0)
end
for _, key in ipairs(input.right_fine) do
    bindings_repeat[key]   = movement_func(opts.fine_movement, 0)
end
for _, key in ipairs(input.up_fine) do
    bindings_repeat[key]      = movement_func(0, -opts.fine_movement)
end
for _, key in ipairs(input.down_fine) do
    bindings_repeat[key]    = movement_func(0, opts.fine_movement)
end
...
mp.add_key_binding(nil, "close-crop", cancel_crop)
mp.add_key_binding(nil, "rev-crop", rev_crop)
mp.add_key_binding(nil, "undo-crop", undo_crop)