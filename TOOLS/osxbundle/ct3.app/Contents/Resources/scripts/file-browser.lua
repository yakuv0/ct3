...
local o = {
    ...
    filter_dot_dirs = true,
    filter_dot_files = true,
    ...
    font_size_header = 30,
    font_size_body = 26,
    font_size_wrappers = 22,

    font_name_header = "unifont",
    font_name_body = "unifont",
    font_name_wrappers = "unifont",
    font_name_folder = "unifont",
    font_name_cursor = "unifont",
...
local function update_ass()
    ...
    if start == 1 and start ~= finish then append(style.footer_header..(#state.list)..' item(s)\\N\\N') end
    --if overflow then append(style.footer_header..(#state.list)..' item(s)\\N\\N') end

    --if  then  append(style.footer_header..(#state.list)..' item(s)\\N\\N') end
    ...
    if #state.list < o.num_entries then
        off = o.num_entries - #state.list
    ...
    if not overflow then append('\\N'..style.footer_header) end
    ass:update()
end
...
local function open()
    ...
    if o.toggle_idlescreen then 
        ...
        --mp.commandv('script-message', 'osc-visibility', 'never', 'no_osc') 
    end
    state.hidden = false
...
local function close()
    ...
    if o.toggle_idlescreen then 
        ...
        --mp.commandv('script-message', 'osc-visibility', 'always', 'osc')
    end
    state.hidden = true
    ass:remove()
end
...
local function loadfile(file, opts)
    ...
    -- Correct: apply mpv.conf window/scale settings after this load only
    mp.add_timeout(0, function()
        -- Force maximized directly
        mp.set_property_native("window-maximized", true)
        local gtfit = mp.get_property("autofit-larger", "")
        if gtfit ~= "" then
            mp.set_property("autofit-larger", gtfit)
        end
        print("____________________",gtfit)
        local ltfit = mp.get_property("autofit-smaller", "")
        if ltfit ~= "" then
            mp.set_property("autofit-smaller", ltfit)
        end
        print("____________________",ltfit)        
        local geometry = mp.get_property("geometry", "")
        if geometry ~= "" then
            mp.set_property("geometry", geometry)
        end
        print(geometry)
        local win_scale = mp.get_property_number("window-scale", 0.77)
        mp.set_property_number("window-scale", win_scale)
        print(win_scale)
    end)
...
state.keybinds = {
    ...
    {'l',       'down_dir',     down_dir},
    {'j',        'up_dir',       up_dir},
    {'k',        'scroll_down',  function() scroll(1, o.wrap) end,           {repeatable = true}},
    {'i',          'scroll_up',    function() scroll(-1, o.wrap) end,          {repeatable = true}},
    {'Shift+j',       'page_down',    function() scroll(o.num_entries) end,       {repeatable = true}},
    {'Shift+l',        'page_up',      function() scroll(-o.num_entries) end,      {repeatable = true}},
    {'Shift+k', 'list_bottom',  function() scroll(math.huge) end},
    {'Shift+i',  'list_top',     function() scroll(-math.huge) end},
    {'`',        'goto_current', goto_current_dir},
    {'Shift+`',  'goto_root',    goto_root},
    {'Ctrl+s',      'reload',       function() cache:clear(); update() end},
    {'s',           'select_mode',  toggle_select_mode},
    {'Shift+s',           'select_item',  toggle_selection},
    {'Alt+s',      'select_all',   select_all},
    {'GAMEPAD_START',            'play-controller',         function() open_file('replace', false) end},
    {'Meta+GAMEPAD_START',       'play_append-controller',  function() open_file('append-play', false) end},
   -- {'Alt+ENTER',              'play_autoload',function() open_file('replace', true) end},
    {'GAMEPAD_BACK',                    'close',        escape},
    {'GAMEPAD_DPAD_RIGHT',       'down_dir-controller',     down_dir},
    {'GAMEPAD_DPAD_LEFT ',       'up_dir-controller',       up_dir},
    {'GAMEPAD_DPAD_DOWN',        'scroll_down-controller',  function() scroll(1, o.wrap) end,           {repeatable = true}},
    {'GAMEPAD_DPAD_UP',          'scroll_up-controller',    function() scroll(-1, o.wrap) end,          {repeatable = true}},
    {'Shift+GAMEPAD_DPAD_RIGHT', 'page_down-controller',    function() scroll(o.num_entries) end,       {repeatable = true}},
    {'Shift+GAMEPAD_DPAD_LEFT',  'page_up-controller',      function() scroll(-o.num_entries) end,      {repeatable = true}},
    {'Shift+GAMEPAD_DPAD_DOWN',  'list_bottom-controller',  function() scroll(math.huge) end},
    {'Shift+GAMEPAD_DPAD_UP',    'list_top-controller',     function() scroll(-math.huge) end},
    {'Meta+GAMEPAD_DPAD_RIGHT',  'goto_current-controller', goto_current_dir},
    {'Meta+GAMEPAD_DPAD_LEFT',   'goto_root-controller',    goto_root},
    --{'Ctrl+s',      'reload',       function() cache:clear(); update() end},
    {'Shift+GAMEPAD_BACK',             'select_mode-controller',  toggle_select_mode},
    --{'Alt+GAMEPAD_BACK',         'select_item-controller',  toggle_selection},
    {'Meta+GAMEPAD_BACK',        'select_all-controller',   select_all}
}
...
mp.add_key_binding('Shift+c','browse-files', toggle)
mp.add_key_binding(nil,'close-bf', close)
--mp.add_key_binding('MENU','open-browser', open)
